...
{
    public class ECartorioRjHealthCheck : IHealthCheck
    {
        private readonly IConfiguration _configuration;

        public ECartorioRjHealthCheck(IConfiguration configuration)
        {
            _configuration = configuration;
            HealthCheckService.TimeOutInMinutes = _configuration["eCartorio:RJ:TimeOutHealthCheck"].ToInt32();
        }
        ...
        public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var urlHost = new Uri(_configuration["eCartorio:RJ:UrlBase"] + "/swagger/index.html");
