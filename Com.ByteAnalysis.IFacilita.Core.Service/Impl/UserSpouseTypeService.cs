using Com.ByteAnalysis.IFacilita.Core.Entity;
using Com.ByteAnalysis.IFacilita.Core.Repository;
using System;
using System.Collections.Generic;
using System.Text;

namespace Com.ByteAnalysis.IFacilita.Core.Service.Impl
{
    public class UserSpouseTypeService : IUserSpouseTypeService
    {
        Repository.IUserSpouseTypeRepository repository;

        public UserSpouseTypeService(IUserSpouseTypeRepository repository)
        {
            this.repository = repository;
        }

        public void Delete(int id)
        {
            this.repository.Delete(id);
        }

        public IEnumerable<UserSpouseType> FindAll()
        {
            return repository.FindAll();
        }

        public UserSpouseType FindById(int id)
        {
            return repository.FindById(id);
        }

        public UserSpouseType Insert(UserSpouseType entity)
        {
            return repository.Insert(entity);
        }

        public UserSpouseType Update(UserSpouseType entity)
        {
            return repository.Update(entity);
        }
    }
}
