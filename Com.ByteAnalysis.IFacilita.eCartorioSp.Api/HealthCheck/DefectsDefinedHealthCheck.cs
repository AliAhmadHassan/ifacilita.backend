...
{
    public class DefectsDefinedHealthCheck : IHealthCheck
    {
        private readonly IConfiguration _configuration;
        private const string serviceName = "DefectsDefinedApi";
        private HealthCheckService currentHealthCheck;

        public DefectsDefinedHealthCheck(IConfiguration configuration)
        {
            ...
            try
            {
                if (!await HealthCheckManager.ExistServiceAsync(serviceName))
                {
                    currentHealthCheck = new HealthCheckService()
                    {
                        ...
                        TimeOutInMinutes = _configuration["Apis:DefectsDefined:TimeOutHealthCheck"].ToInt32()
                    };
                ...
                {
                    var urlHost = new Uri(_configuration["Apis:DefectsDefined:Url"] + "/api/HealthCheck");
    ...
    public class DefectsDefinedSiteHealthCheck : IHealthCheck
    {
        private readonly IConfiguration _configuration;
        private const string serviceName = "DefectsDefinedSite";
        private HealthCheckService currentHealthCheck;

        public DefectsDefinedSiteHealthCheck(IConfiguration configuration)
        {
            _configuration = configuration;
        }
        ...
        public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                using (HttpClient httpClient = new HttpClient(clientHandler))
                {
                    var urlHost = new Uri(_configuration["Apis:DefectsDefined:Site"]);
