...
{
    public class RealOnusHealthCheck : IHealthCheck
    {
        private readonly IConfiguration _configuration;
        private const string serviceName = "RealOnusApi";
        private HealthCheckService currentHealthCheck;

        public RealOnusHealthCheck(IConfiguration configuration)
        {
            ...
            try
            {
                if (!await HealthCheckManager.ExistServiceAsync(serviceName))
                {
                    currentHealthCheck = new HealthCheckService()
                    {
                        ...
                        TimeOutInMinutes = _configuration["Apis:RealOnus:TimeOutHealthCheck"].ToInt32()
                    };
                ...
                {
                    var urlHost = new Uri(_configuration["Apis:RealOnus:Url"] + "/api/HealthCheck");
    ...
    public class RealOnusSiteHealthCheck : IHealthCheck
    {
        private readonly IConfiguration _configuration;
        private const string serviceName = "RealOnusSite";
        private HealthCheckService currentHealthCheck;

        public RealOnusSiteHealthCheck(IConfiguration configuration)
        {
            _configuration = configuration;
        }
        ...
        public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                using (HttpClient httpClient = new HttpClient(clientHandler))
                {
                    var urlHost = new Uri(_configuration["Apis:RealOnus:Site"]);
