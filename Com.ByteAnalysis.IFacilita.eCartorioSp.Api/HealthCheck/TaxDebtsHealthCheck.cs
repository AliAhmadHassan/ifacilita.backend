...
{
    public class TaxDebtsHealthCheck : IHealthCheck
    {
        private readonly IConfiguration _configuration;
        private const string serviceName = "TaxDebtsApi";
        private HealthCheckService currentHealthCheck;

        public TaxDebtsHealthCheck(IConfiguration configuration)
        {
            ...
            try
            {
                if (!await HealthCheckManager.ExistServiceAsync(serviceName))
                {
                    currentHealthCheck = new HealthCheckService()
                    {
                        ...
                        TimeOutInMinutes = _configuration["Apis:TaxDebts:TimeOutHealthCheck"].ToInt32()
                    };
                ...
                {
                    var urlHost = new Uri(_configuration["Apis:TaxDebts:Url"] + "/api/HealthCheck");
    ...
    public class TaxDebtsSiteHealthCheck : IHealthCheck
    {
        private readonly IConfiguration _configuration;
        private const string serviceName = "TaxDebtsSite";
        private HealthCheckService currentHealthCheck;

        public TaxDebtsSiteHealthCheck(IConfiguration configuration)
        {
            _configuration = configuration;
        }
        ...
        public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                using (HttpClient httpClient = new HttpClient(clientHandler))
                {
                    var urlHost = new Uri(_configuration["Apis:TaxDebts:Site"]);
