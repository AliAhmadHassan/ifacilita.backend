...
namespace RoboAntiCaptchaService.Process
{
    public class ProcessServiceBase<TConfig, TEntity> : IProcessServiceBase<TConfig, TEntity> where TConfig : BaseMapperConfig
    {
        ...
        public async Task<ValidationField> ExecuteWrite()
        {
            var countIntent = 1;
            var fillForm = false;
            do
        ...
        public async Task<ValidationField> Submit()
        {
            if (await ClickButton("Submeter"))
            {
                ...
                while (true)
                {
                    ...
                    if (msg.ToLowerInvariant() == "sequência de caracteres inválida")
                    {
                    ...
                    else
                    {
                        if (msg != "wait") // msg of error
                        {
                            ...
                            {
                                case "número de inscrição inválido": return await Task.FromResult(new ValidationField() { Message = msg, Field = "Iptu" });
                                case "escolha uma natureza para prosseguir.": return await Task.FromResult(new ValidationField() { Message = msg, Field = "TransactionNature" });
                                case "cpf/cnpj do adquirente inválido": return await Task.FromResult(new ValidationField() { Message = msg, Field = "Purchaser" });
                                case "cpf/cnpj do transmitente inválido": return await Task.FromResult(new ValidationField() { Message = msg, Field = "Transmitted" });
                                case "informe o nome completo do transmitente.": return await Task.FromResult(new ValidationField() { Message = msg, Field = "PurchaserTransmitted.TransmittedName" });
                                default: return await Task.FromResult(new ValidationField() { Message = msg, Field = "Erro Não Mapeado" });
                            }
                        }
                    }
